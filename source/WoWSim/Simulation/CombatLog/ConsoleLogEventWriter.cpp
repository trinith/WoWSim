...
namespace sim
{
	...
	void ConsoleLogEventWriter::Write(const Simulation& simulation, const DamageEventData& eventData)
	{
		...
		consoleLog.WriteLine(source.GetCharacterIdData().name, " hits ", target.GetCharacterIdData().name, " for ", std::floor(eventData.damageAmount), (eventData.isCritical ? " (crit)" : ""), ".");
	}
